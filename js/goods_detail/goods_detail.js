$(document).ready(function() {
	...
	$('.information .table_view_more').on('click', function() {
		...
		if (isHidden) {
			...
			$('.information tbody tr').eq(5).addClass('tr_hidden');
			$('.information tbody tr').eq(4).addClass('tr_hidden');
			$('.information .table_view_more span').css({
	...
	/* --------------------------分类查看评价-------------------------------- */
	$('.evaluate .evaluate_data .classify span').eq(0).addClass('active');
	...
	$('.evaluate_item .msg_img .msg_img_small').on('click', function() {
		...
		if (isZoom == "zoom-in") {
			...
			$(this).siblings('.msg_img_big').eq(index).css('display', 'block');
			$(this).css('cursor', 'zoom-out');
		}
		if (isZoom == "zoom-out") {
			//还原
			$(this).siblings('.msg_img_big').eq(index).css('display', 'none');
...
window.onload = function() {
	...
	var oPicImg = document.querySelectorAll(".exhibition .pic img");
...
/* --------点击缩略图切换显示的图片------------ */
//全局变量index,图片的索引   index + 1
// var index = 0;
// $('.exhibition .choiceImg .choice').on('click', function() {
// 	index = $(this).index();
// 	$('.exhibition .bigImg img').attr('src', './img/goods_detail/show_big_0' + (index + 1) + '.jpg')
// });
// /* ------------------放大镜------------------ */
// $('.bigImg img').on('mouseenter', function(ev) {
// 	//根据索引index+1改变放大的图片
// 	$('.exhibition .loupe img').attr('src', './img/goods_detail/magnify_0' + (index + 1) + '.jpg');
// 	//显示需要放大的mask
// 	$('.exhibition .bigImg .mask').css('display', 'block');
// 	//显示放大镜区域
// 	$('.exhibition .loupe').css('display', 'block');
// 	//获取鼠标坐标点
// 	var x = ev.pageX - $('.bigImg img').offset().left - $('.exhibition .bigImg .mask').width() / 2;
// 	var y = ev.pageY - $('.bigImg img').offset().top - $('.exhibition .bigImg .mask').height() / 2;
// 	//最小值
// 	console.log(ev);
// 	x = x < 0 ? 0 : x;
// 	y = y < 0 ? 0 : y;
// 	//最大值
// 	x = x > $('.bigImg').width() - $('.exhibition .bigImg .mask').width() ? $('.bigImg').width() - $(
// 		'.exhibition .bigImg .mask').width() : x;
// 	y = y > $('.bigImg').height() - $('.exhibition .bigImg .mask').height() ? $('.bigImg').height() - $(
// 		'.exhibition .bigImg .mask').height() : y;
// 	//移动mask
// 	console.log("鼠标在oShow中的坐标：%d %d", x, y);
// 	$('.exhibition .bigImg .mask').css('left', x + 'px');
// 	$('.exhibition .bigImg .mask').css('top', y + 'px');
// 	//放大镜的图片显示移动
// 	var moveX = ($('.exhibition .loupe_pic').width() - $('.exhibition .loupe').width()) * (x / ($('.bigImg').width() -
// 		$('.exhibition .bigImg .mask').width()));
// 	var moveY = ($('.exhibition .loupe_pic').height() - $('.exhibition .loupe').height()) * (y / ($('.bigImg').height() -
// 		$('.exhibition .bigImg .mask').height()));
// 	$('.exhibition .loupe_pic').css('left', -moveX + "px");
// 	$('.exhibition .loupe_pic').css('top', -moveY + "px");
// });
// $('.bigImg').on('mouseleave', function(ev) {
// 	//隐藏需要放大的mask
// 	$('.exhibition .bigImg .mask').css('display', 'none');
// 	//隐藏放大镜区域
// 	$('.exhibition .loupe').css('display', 'none');
// });
//
// });
